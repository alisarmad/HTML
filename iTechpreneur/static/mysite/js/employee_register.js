...
if(country.length ==0)
{
  ...
  $("#country_error").text('Select Your Location.');
...
$("#country_error").text('');
  ...
  $.ajax({
    ...
    type: "POST",
    data: {'user_type':user_type,'first_name':first_name,'surname':surname,'email':email,'password':password,'technical_subject':technical_subject,'country':country,'csrfmiddlewaretoken':token },
    dataType: 'json',
    cache: false,
     ...
     if(response=='1')
      {
        ...
        $('#employee_sign_up_btn').removeClass("active");
        $('#employee_sign_up_btn').addClass("btn-yellow");
